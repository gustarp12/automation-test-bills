...
import { createIncome, type IncomeActionState } from "./actions";
...
export default function IncomeForm({ currencies, locale }: IncomeFormProps) {
  ...
  return (
    ...
    >
      ...
      {currency && currency !== "DOP" ? (
        <label className="text-sm">
          <span className="text-slate-300">{t(locale, "income.fxRate")}</span>
          <input
            name="fx_rate_to_dop"
            type="number"
            step="0.0001"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="text-sm">
          <span className="text-slate-300">{t(locale, "income.source")}</span>
          <input
            name="source"
            type="text"
            placeholder={t(locale, "income.sourcePlaceholder")}
            className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none focus:border-emerald-400"
          />
        </label>
      ...
      <SubmitButton label={t(locale, "income.addIncome")} />
    </form>
  );
}

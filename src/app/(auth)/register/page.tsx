    ...
    <div className="space-y-8">
      <div className="space-y-2">
        <p className="text-sm uppercase tracking-[0.3em] text-slate-400">
          {t(locale, "auth.createButton")}
        </p>
        <h1 className="text-3xl font-semibold">{t(locale, "auth.createTitle")}</h1>
        <p className="text-sm text-slate-400">{t(locale, "auth.createSubtitle")}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {isLoading ? t(locale, "auth.creating") : t(locale, "auth.createButton")}
        </button>
      </form>
